...
describe("#hasAZero", function(){
    ...
    it("returns false if the number doen't contains at least one 0", function(){
        expect(hasAZero(3332123213112)).toEqual(false)
    })
})

describe('#hasOnlyOddNumbers', function(){
    it('returns true if every single number in the array is odd', function(){
        expect(hasOnlyOddNumbers([1,3,5,7])).toEqual(true)
    })
    it('returns false if  if there is one or more even numbers ', function(){
        expect(hasOnlyOddNumbers([1,3,2,5,7])).toEqual(false)
    })
})

describe('#hasNoDuplicates', function(){
    it('returns true if there are no duplicate values', function () {
        expect(hasNoDuplicates([1,2,3])).toEqual(true)
    })
    it('returns true if there are duplicate values', function () {
        expect(hasNoDuplicates([1,2,3,2])).toEqual(false)
    })
})
